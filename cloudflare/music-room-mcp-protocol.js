...
function sanitize(state, now = Date.now()) {
  ...
  return {
    roomId: String(state?.roomId || ''), revision: Number(state?.revision) || 0,
    playing: Boolean(state?.playing), position, projectedPosition,
    song: state?.song ?? null, updatedAt: state?.updatedAt ?? null, updatedBy: state?.updatedBy ?? null
  };
}
...
async function callTool(env, params) {
  if (params?.name === ROOM_STATE_TOOL.name) {
    const roomId = String(params?.arguments?.roomId || '').trim();
    if (!roomId) return { isError: true, content: [{ type: 'text', text: 'roomId is required' }] };
    return toolResult(sanitize(await env.MUSIC_ROOM.getByName(roomId).getMcpState()));
  }
  if (params?.name === SEARCH_TRACKS_TOOL.name && spotifyConfigured(env)) {
    const query = String(params?.arguments?.query || '').trim();
    if (!query) return { isError: true, content: [{ type: 'text', text: 'query is required' }] };
    const songs = await spotifySearch(env).search(query, { provider: 'spotify', limit: params?.arguments?.limit });
    return toolResult({ provider: 'spotify', query, songs });
  }
...
export async function handleMcpRequest(request, env) {
  ...
  if (message.method === 'tools/call') {
    ...
    catch (cause) { return result(message.id, { isError: true, content: [{ type: 'text', text: `Tool failed: ${cause?.message || 'unknown error'}` }] }); }
  }
