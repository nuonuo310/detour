(() => {
  ...
  async function init(){
    ...
    const receipt=localReceipt||serverReceipt;
    ...
    form.addEventListener('submit',e=>{
      ...
      if(!text)return;
      const next={id:`receipt-${Date.now()}`,foodId:record.id,text,photo:null,createdAt:new Date().toISOString(),readByShenshu:false,source:'local'};
  ...
  function renderReceipt(receipt){
    const existing=document.querySelector('.receipt-existing'),form=document.querySelector('.receipt-form'),state=document.querySelector('[data-receipt-state]'),status=document.querySelector('[data-meta-status]');
    if(!receipt){existing.hidden=true;form.hidden=false;state.textContent='还没有留下回应';status.textContent='等待回执';return;}
    existing.hidden=false;form.hidden=true;state.textContent=receipt.readByShenshu?'哥哥已经看过':'已收到 · 1 条回应';status.textContent=receipt.readByShenshu?'已读':'新回执';
    document.querySelector('[data-receipt-text]').textContent=receipt.text||'（没有文字）';
    document.querySelector('[data-receipt-at]').textContent=`留下于 ${fmt(receipt.createdAt)}`;
  }
  init();
})();
